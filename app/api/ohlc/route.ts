...
async function fetchFromYahooFinance(symbol: string): Promise<any[]> {
  try {
    ...
    const ohlcData = timestamps.map((timestamp: number, index: number) => ({
...
async function fetchFromAlphaVantage(symbol: string): Promise<any[]> {
  ...
  try {
    ...
    const ohlcData = Object.entries(timeSeries).map(([timestamp, values]: [string, any]) => ({
...
export async function GET(request: Request) {
  ...
  try {
    ...
    try {
      ...
      console.warn(`Real data fetch failed for ${instrument}, using fallback:`, error)
