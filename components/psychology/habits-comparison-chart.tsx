      ...
      return (
        <div className={`p-3 rounded-lg border shadow-lg ${
          ...
          <div className="space-y-1 text-xs">
            ...
            <p className={isDark ? 'text-zinc-400' : 'text-slate-600'}>
              Avg P/L: <span className={`font-bold ${data.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                ${data.pnl.toFixed(2)}
              </span>
            </p>
          </div>
      ...
      <CardContent>
        {chartData.length === 0 ? (
          ...
          <>
                ...
                <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 100, bottom: 5 }}>
                  ...
                  <Bar dataKey="winRate" radius={[0, 6, 6, 0]}>
                    {chartData.map((entry, index) => (
                      <Cell 
                        key={`cell-${index}`} 
                        fill={entry.type === 'good' ? '#10b981' : '#ef4444'}
                      />
                    ))}
                  </Bar>
                </BarChart>
            ...
            {/* Legend */}
            <div className="flex items-center justify-center gap-6 mt-6 pt-4 border-t border-slate-200 dark:border-zinc-800">
            ...
            <div className="grid grid-cols-2 gap-4 mt-6">
              <div className={`p-4 rounded-lg border ${
                ...
                <p className={`text-xs mt-1 ${isDark ? 'text-emerald-600' : 'text-emerald-700'}`}>
                  Avg Win Rate
                </p>
              </div>
              ...
              <div className={`p-4 rounded-lg border ${
                ...
                <p className={`text-xs mt-1 ${isDark ? 'text-rose-600' : 'text-rose-700'}`}>
                  Avg Win Rate
                </p>
              </div>
            </div>
