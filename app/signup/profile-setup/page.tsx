...
const stepConfig = [
  {
    ...
    icon: Globe,
    color: "text-purple-400", // Lightened for better contrast
  },
  {
    id: 2,
    title: "Watchlist",
      ...
      <div className="flex items-center gap-3 w-full">
        ...
        <div className="flex flex-col min-w-0">
          <span className={cn(
            ...
            isActive ? "text-foreground" : "text-muted-foreground/70" // Improved contrast for inactive
          )}>
            {step.title}
          </span>
        </div>
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      if (!config.tradingPreferences.primaryInstruments || config.tradingPreferences.primaryInstruments.length === 0) {
        setSaveError("Please select at least one market category to continue.")
        return false
      }
    }
    ...
    if (step === 3) {
      // Assuming userProfile has a username field. 
      // If the object structure differs, adjust the key here.
    ...
    if (step === 6) {
      ...
      if (!termsAccepted || !privacyPolicyAccepted || !dataCollectionConsent) {
        setSaveError("You must accept all mandatory legal agreements to proceed.")
        return false
      }
    }
    ...
    <TooltipProvider>
      <div className="flex h-screen bg-background text-foreground font-sans selection:bg-primary/20">
        ...
        <main className="flex-1 flex flex-col h-full relative bg-background">
          ...
          <div ref={scrollContainerRef} className="flex-1 overflow-y-auto relative z-10 scroll-smooth">
             <div className="max-w-4xl mx-auto px-6 py-12">
                ...
                >
                        ...
                        <div className="relative flex gap-3">
                           <div className="relative flex-1">
                             ...
                             <Input 
                                ...
                                className="pl-10 bg-card/50 border-border/60 h-11 text-foreground placeholder:text-muted-foreground"
                             />
                           </div>
                           <div className="flex border rounded-md overflow-hidden bg-card/50">
                   ...
                   {/* STEP 3: IDENTITY */}
                   {currentStep === 3 && (
                      <div className="max-w-xl bg-card border rounded-xl p-6 backdrop-blur-sm shadow-sm">
                                     ...
                                     <div
                                        ...
                                        className={cn(
                                           ...
                                           isVisible 
                                              ...
                                              : "bg-card/40 border-border/40 hover:bg-muted/20 hover:border-border opacity-70 hover:opacity-100"
                                        )}
                                     >
                                        <div>
          ...
          {/* Footer Controls */}
          <div className="h-24 border-t border-border/40 bg-background/95 backdrop-blur-md flex items-center justify-between px-6 lg:px-12 sticky bottom-0 z-20">
               ...
               <AnimatePresence>
                 {saveError && (
                   <motion.div 
                     ...
                     animate={{ opacity: 1, x: 0 }}
                     exit={{ opacity: 0 }}
                     className="hidden md:flex items-center gap-2 text-destructive text-sm font-medium bg-destructive/10 px-3 py-1.5 rounded-md"
                   >
             ...
             <div className="flex gap-4 items-center">
                ...
                {saveError && (
                   <div className="md:hidden absolute top-2 left-0 w-full px-6 flex justify-center">
                     <div className="text-xs text-destructive bg-destructive/10 px-2 py-1 rounded">{saveError}</div>
                   </div>
                )}
                ...
                <Button 
                   ...
                   className={cn(
                     ...
                     saveError ? "animate-shake bg-primary/90" : "shadow-lg shadow-primary/20 bg-primary hover:bg-primary/90"
                   )}
                >
