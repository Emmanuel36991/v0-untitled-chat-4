...
-- Create fixed function for duration-based analytics
...
BEGIN
  ...
  SELECT 
    dc.duration_category as duration_range,
...
-- Create fixed function for time-based analytics
...
BEGIN
  ...
  WITH time_categories AS (
    SELECT 
      ...
      END as time_category_value
    FROM trades t
    WHERE t.user_id = user_id_param
      AND t.outcome IN ('win', 'loss')
  )
  SELECT 
    tc.time_category_value as time_category,
    COUNT(*)::BIGINT as total_trades,
  ...
  GROUP BY tc.time_category_value
  ORDER BY 
    CASE tc.time_category_value
